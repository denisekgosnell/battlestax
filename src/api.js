...
export const getGame = async (path) => {
  const res = await fetch(`/.netlify/functions/getGame/${path}`);
  return await res.json();
};

export const updateGame = async (path, data) => {
  const res = await fetch(`/.netlify/functions/updateGame/${path}`, {
    method: "PATCH",
...
export const addPlayer = async (gameId, playerId) => {
  const res = await fetch(
    `/.netlify/functions/updateGame/${gameId}/players/${playerId}`,
    {
      method: "PUT",
